...
storage_path = "http://www.google.com/" #must have slash at the end
#todo: specify cloud storage path or serve from cdn?
fullfilename = urljoin(storage_path, "events.json") #path_for_storage_location/event.json
...
class UpdateHandler(webapp.RequestHandler):
    def get(self):
        try:
            _file = urllib.urlretrieve('http://events.hackerdojo.com/events.json', fullfilename)
        except:
    ...
    def get(self):
        ...
        if mobileRedirect == True: #checks if browser is mobile; else shows desktop site
            ...
            try: #get file from online storage location
                response = urllib.urlopen(fullfilename)
